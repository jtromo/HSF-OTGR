    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (_busyRetryTimeout && (numberOfRetries++ > _busyRetryTimeout)) {
                ...
                LogError(@"Database busy, unable to close");
                return NO;
            }
            ...
            if (!triedFinalizingOpenStatements) {
                ...
                while ((pStmt = sqlite3_next_stmt(_db, 0x00)) !=0) {
                    LogTrace(@"Closing leaked statement");
                    sqlite3_finalize(pStmt);
                }
            }
        }
        else if (SQLITE_OK != rc) {
            LogError(@"error closing!: %d", rc);
        }
    }
    while (retry);
...
- (BOOL)rekey:(NSString*)key {
    ...
    if (rc != SQLITE_OK) {
        LogError(@"error on rekey: %d", rc);
        LogError(@"%@", [self lastErrorMessage]);
    }
...
- (void)warnInUse {
    LogError(@"The FMDatabase %@ is currently in use.", self);
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        LogError(@"The FMDatabase %@ is not open.", self);
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_traceExecution && sql) {
        LogTrace(@"%@ executeQuery: %@", self, sql);
    }
    ...
    if (!pStmt) {
        do {
            ...
            if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
                ...
                if (_busyRetryTimeout && (numberOfRetries++ > _busyRetryTimeout)) {
                    LogError(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [self databasePath]);
                    LogError(@"Database busy");
                    sqlite3_finalize(pStmt);
                    _isExecutingStatement = NO;
                    return nil;
                }
                ...
                if (_logsErrors) {
                    LogError(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                    LogError(@"DB Query: %@", sql);
                    LogError(@"DB Path: %@", _databasePath);
#ifndef NS_BLOCK_ASSERTIONS
                    if (_crashOnErrors) {
                        abort();
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            if (namedIdx > 0) {
                ...
                LogError(@"Could not find index for %@", dictionaryKey);
            }
        }
    ...
    if (idx != queryCount) {
        LogError(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return nil;
    }
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_traceExecution && sql) {
        LogTrace(@"%@ executeUpdate: %@", self, sql);
    }
    ...
    if (idx != queryCount) {
        LogError(@"Error: the bind count is not correct for the # of variables (%@) (executeUpdate)", sql);
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return NO;
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (SQLITE_LOCKED == rc) {
                ...
                if (rc != SQLITE_LOCKED) {
                    LogError(@"Unexpected result from sqlite3_reset (%d) eu", rc);
                }
            }
            usleep(20);
            ...
            LogError(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(_db));
            LogError(@"DB Query: %@", sql);
        }
        else if (SQLITE_MISUSE == rc) {
            // uh oh.
            LogError(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(_db));
            LogError(@"DB Query: %@", sql);
        }
        else {
            // wtf?
            LogError(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(_db));
            LogError(@"DB Query: %@", sql);
        }
    ...
    if (closeErrorCode != SQLITE_OK) {
        LogError(@"Unknown error finalizing or resetting statement (%d: %s)", closeErrorCode, sqlite3_errmsg(_db));
        LogError(@"DB Query: %@", sql);
    }
